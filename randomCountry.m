function out = randomCountry(country, mode)
...
switch mode
    case 'weights'
        out.A = rand(size(country.A));
    case 'configurationModel'
        [~, d] = neighbours(country.Eud, country);
        adj = cm_net(d);
        [s, t] = ind2sub(size(adj), find(triu(adj)));
        out.Eud = convert2nodeIdx(country, [s,t]);
